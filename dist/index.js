...
function registerRoutes(app2) {
  ...
  app2.post("/admin/login", async (c) => {
    ...
    try {
      await env.DB.prepare("INSERT OR REPLACE INTO admin_session (id, token_hash, expires_at) VALUES (?, ?, ?)").bind("default", hashHex, Math.floor(Date.now() / 1e3) + 86400).run();
    } catch (e) {
  ...
  app2.get("/admin", async (c) => {
    ...
    const allowed = await isAdminRequest(c);
  ...
  app2.get("/admin/api/stats", async (c) => {
  ...
  app2.get("/admin/api/users", async (c) => {
    if (!await isAdminRequest(c))
      return c.json({ error: "Forbidden" }, 403);
  ...
  app2.post("/admin/api/users", async (c) => {
    if (!await isAdminRequest(c))
      return c.json({ error: "Forbidden" }, 403);
  ...
  app2.put("/admin/api/users/:uuid", async (c) => {
    if (!await isAdminRequest(c))
      return c.json({ error: "Forbidden" }, 403);
    ...
    if (!exp_date || !exp_time)
      return c.json({ error: "Missing date/time" }, 400);
    try {
  ...
  app2.delete("/admin/api/users/:uuid", async (c) => {
    if (!await isAdminRequest(c))
      return c.json({ error: "Forbidden" }, 403);
    const uuid = c.req.param("uuid");
    try {
  ...
  app2.post("/admin/api/health-check", async (c) => {
    if (!await isAdminRequest(c))
      return c.json({ error: "Forbidden" }, 403);
    const env = c.env;
    const proxyIps = env.PROXYIPS ? env.PROXYIPS.split(",").map((s) => s.trim()) : ["speed.cloudflare.com:443"];
    const stmts = [];
    for (const ipPort of proxyIps) {
      ...
      stmts.push(env.DB.prepare("INSERT OR REPLACE INTO proxy_health (ip_port, is_healthy, latency_ms, last_check) VALUES (?, ?, ?, ?)").bind(ipPort, healthy, latency, Math.floor(Date.now() / 1e3)));
    }
    try {
      await env.DB.batch(stmts);
  ...
  app2.post("/admin/logout", async (c) => {
    try {
      await c.env.DB.prepare("DELETE FROM admin_session WHERE id = ?").bind("default").run();
    } catch (e) {
    }
    const headers = new Headers();
  ...
  app2.get("/api/user/:uuid/stats", async (c) => {
    const { uuid } = c.req.param();
    const env = c.env;
    if (!/^[0-9a-fA-F-]{36}$/.test(uuid))
      return c.json({ error: "invalid" }, 400);
    const row = await env.DB.prepare("SELECT traffic_used, traffic_limit, expiration_date, expiration_time FROM users WHERE uuid = ?").bind(uuid).first();
    if (!row)
  ...
  app2.all("/ws", async (c) => {
    ...
    const flushUsage = async () => {
      try {
        if (!uuidStr || sessionUsage <= 0)
          return;
        c.executionCtx.waitUntil((async () => {
          try {
            await env.DB.prepare("UPDATE users SET traffic_used = traffic_used + ? WHERE uuid = ?").bind(Math.round(sessionUsage), uuidStr).run();
          } catch (e) {
            console.error("flush usage error", e);
          }
        })());
        sessionUsage = 0;
      } catch (e) {
        console.error("flushUsage outer", e);
      }
    };
      ...
      try {
        ...
        if (!uuidStr) {
          try {
            ...
            uuidStr = parsed.uuid;
            const userRow = await env.DB.prepare("SELECT uuid, traffic_limit, traffic_used, expiration_date, expiration_time, ip_limit FROM users WHERE uuid = ?").bind(uuidStr).first();
            if (!userRow) {
              server.close(1011, "Authentication failed");
              return;
            }
            const expIso = expiryToISO(userRow.expiration_date, userRow.expiration_time);
            if (expIso && new Date(expIso) <= /* @__PURE__ */ new Date()) {
            ...
            try {
              ...
              c.executionCtx.waitUntil((async () => {
                try {
                  await env.DB.prepare("INSERT OR REPLACE INTO user_ips (uuid, ip, last_seen) VALUES (?, ?, CURRENT_TIMESTAMP)").bind(uuidStr, req.headers.get("CF-Connecting-IP") || "unknown").run();
                } catch (e) {
                  console.warn("user_ip upsert failed", e);
                }
              })());
            } catch (e) {
              console.warn("ip_limit check err", e);
            }
          } catch (e) {
        ...
        try {
          ...
          console.warn("send ack failed", e);
        }
      } catch (err) {
